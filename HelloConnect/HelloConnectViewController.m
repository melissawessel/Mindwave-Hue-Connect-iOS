...
- (void)viewDidLoad
{
    ...
    heading.text = @"Press Scan to look for your device";
...
-(void)didConnect
{
    ...
    heading.text = @"Device connected";
}

-(void)didDisconnect
{
    heading.text = @"Disconnected";
    NSLog(@"%s", __func__);
}
...
-(void)eSense:(int)poorSignal Attention:(int)attention Meditation:(int)meditation
{
    ...
    NSString* attStr = [@(attention) stringValue];
    float attFloat = attention;
    heading.text = [@"Attention: " stringByAppendingString:attStr];
    self.view.backgroundColor = [UIColor colorWithRed:255/255 green:(100-attFloat)/100 blue:(100-attFloat)/100 alpha:1];
